...
/*   Updated: 2025/05/22 15:21:17 by efinda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // FIRST ROW
    tiles[0][0].up = &tiles[size.y - 1][0];
    tiles[0][0].down = &tiles[1][0];
    tiles[0][size.x - 1].up = &tiles[size.y - 1][size.x - 1];
    tiles[0][size.x - 1].down = &tiles[1][size.x - 1];
    // LAST ROW
    tiles[size.y - 1][0].up = &tiles[size.y - 2][0];
    tiles[size.y - 1][0].down = &tiles[0][0];
    tiles[size.y - 1][size.x - 1].up = &tiles[size.y - 2][size.x - 1];
    tiles[size.y - 1][size.x - 1].down = &tiles[0][size.x - 1];
    while (++y < size.y)
	{
        tiles[y][0].right = &tiles[y][1];
        tiles[y][0].left = &tiles[y][size.x - 1];
        tiles[y][size.x - 1].right = &tiles[y][0];
